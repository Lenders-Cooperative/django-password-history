...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-password-history runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-password-history.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ django_password_history")
...
def lint(c):
    ...
    """
    c.run("flake8 django-password-history tests")
...
def release(c, bumpsize=''):
    ...
    import django_password_history
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_password_history.__version__))
    c.run("git push --tags")
